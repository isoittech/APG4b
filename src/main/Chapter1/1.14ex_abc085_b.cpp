...
int main()
{
  ...
  // ------------------------------------------
  // // 小さい順にソート
  // sort(chokkeiDatas.begin(), chokkeiDatas.end());
  // tsumareruMochi.push_back(chokkeiDatas.at(0));

  // for (int i = 1; i < N; i++)
  // {
  //   if (chokkeiDatas.at(i) == chokkeiDatas.at(i - 1))
  //   {
  //     continue;
  //   }
  ...
  // ------------------------------------------
  sort(chokkeiDatas.begin(), chokkeiDatas.end());
  chokkeiDatas.erase(unique(chokkeiDatas.begin(), chokkeiDatas.end()), chokkeiDatas.end());

  cout << "答え（餅の段数）:" << chokkeiDatas.size() << endl;
}